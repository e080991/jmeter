...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/testResults">
        <html lang="en">
            <head>
                <meta name="Author" content="shanhe.me, klatt.ie"/>
                ...
                * { margin: 0; padding: 0 }
                ...
                td { vertical-align: baseline; font-size: 12px; max-width: 1300px; }
                ...
                #right-panel .data { line-height: 19px; white-space: nowrap; word-wrap: break-word; word-break: break-all; }
                #right-panel pre.data { white-space: pre-wrap; word-wrap: break-word; word-break: break-all; }
                ...
                var onclick_li = (function() {
                    ...
                    return function(li) {
                        var depth = (parseInt(li.firstChild.style.paddingLeft) - 25) / 10;
                        if(depth == 0) {
                            var next_li;
                            var do_run = true;
                            next_li = li.nextSibling;
                            while(do_run) {
                                if(next_li) {
                                    var next_depth = (parseInt(next_li.firstChild.style.paddingLeft) - 25) / 10;
                                    if(next_depth == 0) {
                                        do_run = false;
                                    } else {
    ...
    <xsl:template match="sample|httpSample">
        <xsl:param name="depth"/>
        <li onclick="return onclick_li(this);">
            <xsl:attribute name="style">display:
                <xsl:choose>
                    <xsl:when test="$depth = 0">block</xsl:when>
            ...
            <div>
                ...
                <xsl:attribute name="style">background-position: <xsl:value-of select="5 + $depth*10"/>px 3px;
                    padding: 0 0 0 <xsl:value-of select="25 + $depth*10"/>px;
                    <xsl:choose>
                        <xsl:when test="$depth = 0">background-color: transparent;</xsl:when>
                    </xsl:choose>
                </xsl:attribute>
                ...
                <div class="zebra">
                    <table>
                        <tr>
                            ...
                            <td class="data">
                                <pre class="data">
                                    <xsl:value-of select="method"/><xsl:text> </xsl:text><xsl:value-of
                                        select="java.net.URL"/>
                                </pre>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td class="data key">Request Data</td>
                            <td class="data delimiter">:</td>
                            <td class="data">
                                <pre class="data">
